...
def forecast(request: ForecastRequest):
    try:
        ...
        if user_date <= last_date:
            # Take available known values from dataset
            known_subset = df_resampled[df_resampled["Date"] >= user_date].head(7)

            # If we already got 7 days fully inside dataset → return them
            if len(known_subset) == 7:
                return known_subset[["Date", target_col]].rename(columns={target_col: "Yield"}).to_dict(orient="records")

            # Otherwise, we need to extend with forecast
            days_missing = 7 - len(known_subset)

            # Forecast future values starting from last_date+1
        ...
        # --- Case B: User date beyond dataset entirely ---
            ...
            # Take 7 days starting from user_date
